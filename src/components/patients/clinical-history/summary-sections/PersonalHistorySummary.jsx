...
export default function PersonalHistorySummary({ data }) {
  if (data.length === 0) {
    return <p className="text-gray-500 text-sm">No se han registrado antecedentes patológicos</p>
  }
    ...
    <div className="space-y-3">
      {data.map((item, index) => (
        <div key={index} className="text-sm border-l-4 border-blue-200 pl-3">
          <div className="font-medium text-blue-800">{item.disease}</div>
          {item.duration_value && (
            <div className="text-gray-600 mt-1">
              <span className="text-xs font-medium">Tiempo con la enfermedad:</span> {item.duration_value} {item.duration_unit}
            </div>
          )}
